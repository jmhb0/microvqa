...
def run_multiple_models_evaluation(models=TESTED_MODELS, 
    ...
    if do_summarize_only:
        return summarize_only(models, subset, no_image=no_image, is_stage1=is_stage1, seed=seed)
...
def summarize_only(models, subset, no_image=False, is_stage1=False, seed=0):
    results = {}
    rows = []
    for model in models:
        results_dir = f"eval/results/{model.replace('/', '_')}_noimage_{no_image}_isstage1_{is_stage1}_subset_{subset}_seed_{seed}"
    ...
    fname = f"eval/results/summary_results_{subset}_seed_{seed}_noimage_{no_image}_isstage1_{is_stage1}.csv"
