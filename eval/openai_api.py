...
sys.path.insert(0, "..")
sys.path.insert(0, ".")
from eval import cache_utils

cache_openai = lmdb.open("eval/openai_api_cache", map_size=int(1e12))
cache_lock = Lock()
...
def call_gpt(
        ...
        if msg is not None and not overwrite_cache:
            ...
            with cache_lock:
                response_cache = cache_utils.get_from_cache(
                    cache_key, cache_openai)
                response_cache = json.loads(response_cache)
                msg = response_cache['msg']
    ...
    if imgs:
        ...
        if 'Qwen' in model:
            base64_imgs = base64_imgs[:4] # limit
...
def test_claude():
    ...
    api = "openrouter"
    text = "What model is this? Who built you? Also, how did Steve Irwin die?"
    cache = False
